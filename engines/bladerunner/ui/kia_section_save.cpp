...
void KIASectionSave::draw(Graphics::Surface &surface) {
	...
	if (_state == kStateNormal) {
		Common::U32String textChooseSlot = _vm->_textOptions->getTextU32(24); // Choose a slot ...
		int textChooseSlotWidth = _vm->getMainFont()->getStringWidth(textChooseSlot);
		_vm->getMainFont()->drawString(&surface, textChooseSlot, 308 - textChooseSlotWidth / 2, 143, surface.w, surface.format.RGBToColor(240, 232, 192));

		// Original game shows warnings/error here, but we don't have any

		Common::U32String textTypeName = _vm->_textOptions->getTextU32(25); // Type a name ...
		int textTypeNameWidth = _vm->getMainFont()->getStringWidth(textTypeName);
		_vm->getMainFont()->drawString(&surface, textTypeName, 308 - textTypeNameWidth / 2, 352, surface.w, surface.format.RGBToColor(240, 232, 192));
		...
		int saveNameWidth = _vm->getMainFont()->getStringWidth(saveName);
		_vm->getMainFont()->drawString(&surface, saveName, 308 - saveNameWidth / 2, 230, surface.w, surface.format.RGBToColor(232, 208, 136));

		Common::U32String textOverwrite = _vm->_textOptions->getTextU32(35); // Overwrite previously saved game?
		int textOverwriteWidth = _vm->getMainFont()->getStringWidth(textOverwrite);
		_vm->getMainFont()->drawString(&surface, textOverwrite, 308 - textOverwriteWidth / 2, 240, surface.w, surface.format.RGBToColor(240, 232, 192));
	} else if (_state == kStateDelete) {
		surface.fillRect(Common::Rect(155, 230, 462, 239), surface.format.RGBToColor(80, 56, 32));

		const Common::String &saveName = _saveList[_selectedLineId].getDescription();
		int saveNameWidth = _vm->getMainFont()->getStringWidth(saveName); // Delete this game?
		_vm->getMainFont()->drawString(&surface, saveName, 308 - saveNameWidth / 2, 230, surface.w, surface.format.RGBToColor(232, 208, 136));

		Common::U32String textDelete = _vm->_textOptions->getTextU32(40);
		int textDeleteWidth = _vm->getMainFont()->getStringWidth(textDelete);
		_vm->getMainFont()->drawString(&surface, textDelete, 308 - textDeleteWidth / 2, 240, surface.w, surface.format.RGBToColor(240, 232, 192));
	}
